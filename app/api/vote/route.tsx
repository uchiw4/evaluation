...
async function visite(
  ...
  const sql_verif = `SELECT * FROM vote WHERE user = ? AND oeuvre = ?`;
  const verif = await db.get(sql_verif, user, oeuvre);
  ...
  // Insert the new visit
    const now = new Date()
    const sql = `INSERT INTO vote (user, oeuvre, date) VALUES (?, ?, ?)`;
